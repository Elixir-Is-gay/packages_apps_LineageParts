...
public class ButtonSettings extends SettingsPreferenceFragment
    ...
    private void applyCardLayout() {
        PreferenceCategory mNavBarPrefCat = (PreferenceCategory) findPreference("navigation_bar_category");
        PreferenceCategory mPowerPrefCat = (PreferenceCategory) findPreference("power_key");
        PreferenceCategory mHomePrefCat = (PreferenceCategory) findPreference("home_key");
        PreferenceCategory mBackKeyPrefCat = (PreferenceCategory) findPreference("back_key");
        PreferenceCategory mMenuKeyPrefCat = (PreferenceCategory) findPreference("menu_key");
        PreferenceCategory mAssistKeyPrefCat = (PreferenceCategory) findPreference("assist_key");
        PreferenceCategory mAppSwitchPrefCat = (PreferenceCategory) findPreference("app_switch_key");
        PreferenceCategory mCameraKeyPrefCat = (PreferenceCategory) findPreference("camera_key");
        PreferenceCategory mVolumeKeyPrefCat = (PreferenceCategory) findPreference("volume_keys");
        PreferenceCategory mExtraPrefCat = (PreferenceCategory) findPreference("extras_category");
        applyCardLayoutToCategory(mNavBarPrefCat);
        applyCardLayoutToCategory(mPowerPrefCat);
        applyCardLayoutToCategory(mHomePrefCat);
        applyCardLayoutToCategory(mBackKeyPrefCat);
        applyCardLayoutToCategory(mMenuKeyPrefCat);
        applyCardLayoutToCategory(mAssistKeyPrefCat);
        applyCardLayoutToCategory(mAppSwitchPrefCat);
        applyCardLayoutToCategory(mCameraKeyPrefCat);
        applyCardLayoutToCategory(mVolumeKeyPrefCat);
        applyCardLayoutToCategory(mExtraPrefCat);

    }

    private void applyCardLayoutToCategory(PreferenceCategory mPrefCategory) {
        if (mPrefCategory == null || mPrefCategory.getPreferenceCount() == 0) {
            Log.e(TAG, "mPrefCategory is null!");
            return;
        }
        if (mPrefCategory.getPreferenceCount() == 1) {
            final Preference preference = mPrefCategory.getPreference(0);
